...
export default function RoomRestockingPage({ isManager }: { isManager: boolean }) {
  ...
  async function setOrdered(id: string, action: "mark_ordered" | "reopen") {
    setBusyId(id);
    try {
      const res = await fetch(`/api/inventory/room-restocking/${id}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action }),
      });
      if (res.ok) {
        setLogs((prev) => prev.map((l) => (l.id === id ? { ...l, ordered: action === "mark_ordered" } : l)));
      }
    } finally {
      setBusyId(null);
    }
