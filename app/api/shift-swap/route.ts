...
export async function POST(req: NextRequest) {
  ...
  const { targetEmployeeId, shiftDescription } = await req.json();
  ...
  if (typeof shiftDescription !== "string" || !shiftDescription.trim()) {
    return NextResponse.json({ error: "Missing shiftDescription." }, { status: 400 });
  }
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      shift_description: shiftDescription.trim(),
