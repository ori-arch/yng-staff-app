...
export async function GET() {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, item_type, specific_item, remaining_quantity, empty_bottle_photo_url, new_item_photo_url, created_at, employees(name), room_ran_out:room_ran_out_id(name), room_restocked:room_restocked_id(name)"
    )
...
export async function POST(req: NextRequest) {
  ...
  if (!(newItemPhoto instanceof File) || newItemPhoto.size === 0) {
  ...
  const { error: insertError } = await supabase.from("room_restocking_logs").insert({
